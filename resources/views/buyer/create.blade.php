    ...
    <div class="card shadow-lg p-3">
        <form action="{{ route('buyer.store') }}" method="post" class="form">
            ...
            <div class="mb-3">
                ...
                <input class="form-control @error('contact') is-invalid @enderror" type="text" id="contact" name="contact" required value="{{ old('contact') }}">
                @error('contact')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                    ...
                    <option value="Koperasi" @selected(old('type') == 'Koperasi')>Koperasi</option>
                    <option value="Pasar Lokal" @selected(old('type') == 'Pasar Lokal')>Pasar Lokal</option>
                    <option value="Supermarket" @selected(old('type') == 'Supermarket')>Supermarket</option>
                </select>
                @error('type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
